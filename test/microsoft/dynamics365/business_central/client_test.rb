...
require 'test_helper'
# rake test TEST=test/microsoft/dynamics365/business_central/client_test.rb

class Microsoft::Dynamics365::BusinessCentral::ClientTest < Minitest::Test
  DEFAULT_LOGIN_URL = Microsoft::Dynamics365::BusinessCentral::Client::DEFAULT_LOGIN_URL
  DEFAULT_URL = Microsoft::Dynamics365::BusinessCentral::Client::DEFAULT_URL
  ...
  def test_request_client_token
    ...
    stub_request(:post, /#{DEFAULT_LOGIN_URL}/)
      .to_return(
        status: 200,
        headers: {
  ...
  def test_authorize_throws_exception
    stub_request(:get, DEFAULT_LOGIN_URL)
      .to_return(status: 200, body: '', headers: {})

    mock = MiniTest::Mock.new
    def mock.authorize_url(_arguments)
      response = Faraday.get(DEFAULT_LOGIN_URL)
  ...
  def test_request_token_throws_exception
    ...
    def mock.get_token(*_params)
      response = Faraday.get(DEFAULT_URL)
      response = OAuth2::Response.new(response)
      raise OAuth2::Error, response
    end
